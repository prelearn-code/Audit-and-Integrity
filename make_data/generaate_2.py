...
def test_single_request():
    test_prompt = r"""阅读后面的文本，然后提取出一个英文关键词，并只输出一个关键词，表示文章的主题。除了特定的人名与地名，都用小写表示。并且只输出为一个单一的英文单词，不能出现中文，或者其他多余的符号，只要一个单一的单词。
Message-ID: <31599077.1075849627629.JavaMail.evans@thyme>
Date: Mon, 27 Nov 2000 08:56:00 -0800 (PST)
...
X-cc: 
X-bcc: 
X-Folder: \John_Arnold_Nov2001\Notes Folders\Avaya
X-Origin: ARNOLD-J
...
def main():
    print("=" * 60)
    print("Ollama 性能测试 - RTX 5060 Ti 16G + qwen2.5:7b")
    print("=" * 60)
    
    print(f"\n正在测试模型: {MODEL}")
    print("测试5次请求，计算平均响应时间...\n")
    
    times = []
    for i in range(5):
        elapsed, result = test_single_request()
        if elapsed:
            times.append(elapsed)
            print(f"第 {i+1} 次: {elapsed:.2f}秒 -> 关键词: {result}")
        else:
            print(f"第 {i+1} 次: 失败 - {result}")
    
    if times:
        avg_time = sum(times) / len(times)
        print("\n" + "=" * 60)
        print(f"平均响应时间: {avg_time:.2f} 秒/请求")
        print(f"\n推荐配置:")
        print(f"  MAX_WORKERS = {min(20, max(10, int(15/avg_time)))}")
        print(f"    (基于你的GPU，建议并发数在 10-20 之间)")
        print(f"\n预计处理5000个文件需要: {(5000 * avg_time / min(20, max(10, int(15/avg_time)))) / 60:.1f} 分钟")
        print("=" * 60)
    else:
        print("\n测试失败！请检查ollama是否正在运行。")
